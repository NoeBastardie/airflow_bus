...
bucket_name = 'etl-airflow-train-pos-bucket'
...
def extract_API():
    ...
    conn.request("GET", "/TrainPositions/TrainPositions?contentType={json}&%s" % params, "{body}", headers)
    response = conn.getresponse()
    data = response.read()
    data_str = data.decode()
    data_dict = json.loads(data_str)
    df = json_normalize(data_dict["TrainPositions"])
    return df
